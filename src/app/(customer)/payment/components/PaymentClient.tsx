...
import { showWaringMessage } from "@/app/utils/alertHelper";
...
const PaymentClient = () => {
  ...
  useEffect(() => {
    const fetchFoodItems = async () => {
      ...
      try {
        setIsLoading(true);
        const foodPromises = Object.entries(bookingData.food).map(
          async ([id, data]) => {
            try {
              const response = await axiosInstance.get(
                `/theater-food/${data.id}`
              );
              const foodItem: FoodItem = response.data.data;
              return {
                id: data.id,
                name: data.name,
  ...
  const ticketPrice =
    bookingData?.seats.reduce(
      (total, seat) => total + (seat.price || 100000),
      0
    ) || 0;
  ...
  const handlePayment = async () => {
    if (!selectedPaymentMethod) {
      showWaringMessage("Vui lòng chọn phương thức thanh toán");
      return;
    }
    ...
    try {
      ...
      const queryParams = new URLSearchParams({
        ...
        seats:
          bookingData?.seats
            .map((seat) => `${seat.seatName}:${seat.price}`)
            .join(",") || "",
        food:
          foodOrders
            .map(
              (food) =>
                `${food.id}:${encodeURIComponent(food.name)}:${food.quantity}`
            )
            .join(",") || "",
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="font-medium text-gray-900">
                  {bookingData.showtime
                    ? new Date(bookingData.showtime).toLocaleTimeString(
                        "vi-VN",
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="font-medium text-gray-900">
                  {bookingData.seats.map((seat) => seat.seatName).join(", ")}
                </p>
              </div>
            </div>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {order.price.toLocaleString("vi-VN")}đ x{" "}
                          {order.quantity}
                        </p>
                      </div>
            ...
            <div className="flex justify-between items-center py-3">
              <div>
                <span className="text-gray-700">
                  Vé xem phim ({bookingData?.seats.length} vé)
                </span>
